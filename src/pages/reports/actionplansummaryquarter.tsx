...
const ActionPlanSummaryQuarterReportPage: NextPage<ActionPlanSummaryQuarterReportProps> = props => {
  const router = useRouter();
  const {year, start, end} = router.query;
  const [loading, setLoading] = useState<boolean>(true);
  const [data, setData] = useState<any>();
  const { getReportProps } = useReports();
  const fetchData = async () => {
    const actionPlanProps = await getReportProps(ReportType.ACTION_PLAN_SUMMARY_QUARTER);
    Promise.resolve(actionPlanProps);
    console.log('actionPlanProps  :', actionPlanProps);
    setData(actionPlanProps);
    setLoading(false);
  };
  useMountEvents({
    onMounted: async () => {
      ...
      {loading ? (
        ...
        <DashboardWrapper tab={NavigationTab.REPORTS}>
          <Grid container spacing={1}>
            <Grid xs={3}></Grid>
            <Grid item xs={6}>
              <ActionPlanSummaryQuarterReport
...
export default ActionPlanSummaryQuarterReportPage;
