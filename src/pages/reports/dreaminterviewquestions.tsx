...
const DreamInterviewQuestionsReportPage = () => {
  const [loading, setLoading] = useState<boolean>(true);
  const [data, setData] = useState<any>();
  const { getReportProps } = useReports();
  const fetchData = async () => {
    const dreaminterviewquestions = await getReportProps(ReportType.DREAM_INTERVIEW);
    Promise.resolve(dreaminterviewquestions);
    console.log('dreaminterviewquestions  :', dreaminterviewquestions);
    setData(dreaminterviewquestions);
    setLoading(false);
  };
  useMountEvents({
    onMounted: async () => {
...
export default DreamInterviewQuestionsReportPage;
