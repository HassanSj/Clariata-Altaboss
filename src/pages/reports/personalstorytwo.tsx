...
const PersonalStoryTwoReportPage = () => {
  const [loading, setLoading] = useState<boolean>(true);
  const [data, setData] = useState<any>();
  const { getReportProps } = useReports();
  const fetchData = async () => {
    const personalStoryTwoProps = await getReportProps(ReportType.PERSONAL_STORY_2);
    Promise.resolve(personalStoryTwoProps);
    setData(personalStoryTwoProps);
    setLoading(false);
  };
...
export default PersonalStoryTwoReportPage;
