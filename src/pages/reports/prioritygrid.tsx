...
import PriorityGridReport, {
  getPriorityGridReportProps,
  PriorityGridReportProps,
} from '~/ui/components/Reports/PriorityGridReport/PriorityGridReport';
      ...
      {loading ? (
        ...
        <DashboardWrapper tab={NavigationTab.REPORTS}>
          <Grid container spacing={1}>
            <Grid xs={3}></Grid>
            <Grid item xs={6}>
              <PriorityGridReport household={data?.household} persons={data?.persons} objectives={data?.objectives} />
            </Grid>
          </Grid>
        </DashboardWrapper>
      )}
...
// export const getServerSideProps: GetServerSideProps<PriorityGridReportProps> = async (context) => {
//   if (context.query) {
//     request.private = createServerClientWithToken(context.req, String(context.query.token));

//     const data = await getPriorityGridReportProps(Number(context.query.householdId), Number(context.query.interviewId));
...
// // @ts-ignore
// PriorityGridReportPage.Layout = PDFLayout;

export default PriorityGridReportPage;
