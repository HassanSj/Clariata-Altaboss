...
import ObjectiveTemplate from "~/ui/components/ActionItems/CellTemplates/ObjectiveTemplate";
import StatusTemplate from "~/ui/components/ActionItems/CellTemplates/StatusTemplate";
import ActionItemsTemplate from "~/ui/components/ActionItems/CellTemplates/ActionItemsTemplate";
...
const ActionItem = ({ item, index, onChange }: IProps): ReactElement => {
  const notifications = useNotifications();
  const { actionItemStatuses } = useStoreState(state => state.constants);
  //const { data: actionItemStatuses } = useSWR<ActionItemStatus>([`${process.env.NEXT_PUBLIC_API_URL}/phonenumbertype/list`, getAccessToken()], fetcher);
    ...
    <>
      ...
      { showAddSubDialog ?
        <EditActionItem parent={item}
                        ...
                        onClose={() => setShowAddSubDialog(false)}></EditActionItem>
        : null }
      { showEditDialog ?
