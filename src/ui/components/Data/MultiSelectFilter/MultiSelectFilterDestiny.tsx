...
import { CategoryItem } from "~/ui/pages/Destiny/Destiny";
...
const MultiSelectFilter = ({id = 'multiselectfilter',
  ...
  const [values, setValues] = React.useState<any | string[]>();
  ...
  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checkIsChecked(event.target.name)) {
      ...
      const newValues = Object.keys(newState)?.map((k: string) => String(k));
      setState(newState);
      setValues(newValues);
      if (onUpdate) {
