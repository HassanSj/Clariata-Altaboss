...
import {DialogActions, Button as MuiButton, FormControlLabel, Checkbox, Grid, Button, Icon, Divider} from "@material-ui/core";
...
const TimelineEditor = ({ isOpen, onClose }: IProps): ReactElement => {

  const report = ReportTypes[ReportType.ANCESTRAL_TIMELINE];
  // const { downloadCustomTimelineReport } = useReports();
  const { householdId, primary1Id, primary2Id } = useStoreState((state) => state.selected);
  const [loading, setLoading] = useState<boolean>(false);


  const [primaryOne, setPrimaryOne] = React.useState<Person>();
  const [primaryTwo, setPrimaryTwo] = React.useState<Person>();
  ...
  const [includeCustom, setIncludeCustom] = React.useState(true);
  ...
  // const [selectablePersons, setSelectablePersons] = React.useState<Person[]>([]);
  ...
  //   const timelinereport = await downloadCustomTimelineReport(excludePrimary,excludeGrandParents,excludeParents,excludeChildren,excludeGrandChildren,excludeBirth,excludeDeath,excludeMarriage,excludeGraduation,excludeCustom,startDate ? moment(startDate).year() : undefined,endDate ? moment(endDate).year() : undefined);
  //   setProps(timelinereport?.props);
  //   setParams(timelinereport?.params);
  //   setDownloadUrl(timelinereport?.url);
  //   setShowReport(true);
  // };
  ...
  const runReport = async () => {
    ...
    let data = await getTimelineData(selectedHousehold.HouseholdID, startYear, endYear, filters, customFilter);

    console.log('Beginning');
    console.log(startYear);

    setTimelineData(data.timelineitems);
    //console.log(data.primaryOne);
    setPrimaryOne(data.primaryOne);
    //console.log(data.primaryTwo);
    setPrimaryTwo(data.primaryTwo);
  ...
  return (
    <>
      <Modal title={modalHeader()} isOpen={isOpen} handleClose={handleClose} width="lg" hideFooter={true}>
              ...
              <Grid container item spacing={1}>
                ...
                <Grid item xs={2}>
                  <div style={{ paddingTop: '25px' }}>
                    <Button size="large" variant="contained" color="primary" onClick={() => runReport()}>
                      View Timeline
                    </Button>
                  </div>
                </Grid>
          ...
          {showAddItem ? 
          <>
            <TimelineDialog isOpen={showAddItem} onClose={CloseAddItem}  onSave={AddTimelineItem}  />
          </>
          : null }
          { showReport ? 
          <>
            <TimelineReport timelineItems={timelineData} household={household} persons={persons} isModal={false} primaryOne={primaryOne} primaryTwo={primaryTwo}/>
          </>
            : null }
      </Modal>
    </>
